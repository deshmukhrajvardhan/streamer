...
def get_mpd(url):
    ...
    try:
        ...
        connection_network_buffer_size= DOWNLOAD_CHUNK
    ...
    try:
        ...
        with open(segment_filename,'wb') as segment_file_handle:
                for segment_data in seg_conn.read_chunked_give_size(DOWNLOAD_CHUNK):
                    ...
                    with open('/mnt/QUIClientServer0/data_read_mod_chunk_hyper_HTTP1','ab') as movie:
...
def start_playback_smart(dp_object, domain, playback_type=None, download=False, video_segment_duration=None, retrans=False):
    ...
    while segment_number < len(dp_list):
        ...
        segment_download_rate=segment_size/segment_download_time
        with open('/mnt/QUIClientServer0/hyper_http1_read_mod_chunk_seg_time_rate.txt', 'a') as rate_f:
