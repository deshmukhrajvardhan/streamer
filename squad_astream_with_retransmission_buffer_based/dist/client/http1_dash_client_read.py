...
def download_segment(segment_url, dash_folder):
    ...
    try:
        ...
        with open(segment_filename,'wb') as segment_file_handle:
                    segment_data=seg_conn.read()
                    #if segment_data is None:
                    #    break
                    with open('/mnt/QUIClientServer0/data_read_hyper_HTTP1','ab') as movie:
                        movie.write(segment_data)
                    segment_file_handle.write(segment_data)
                    segment_size += len(segment_data)
                    #with open("/mnt/QUIClientServer0/REQUESTS-chunks-data-http1",'a') as rread:
                    #    rread.write(str(segment_data))
                    ...
                    with open('/mnt/QUIClientServer0/chunk_rate_read_squad_hyper_HTTP1.txt', 'a') as chk:
...
def start_playback_smart(dp_object, domain, playback_type=None, download=False, video_segment_duration=None, retrans=False):
    ...
    while segment_number < len(dp_list):
        ...
        segment_download_rate=segment_size/segment_download_time
        with open('/mnt/QUIClientServer0/hyper_http1_read_seg_time_rate.txt', 'a') as rate_f:
