...
WriteMemoryCallback2(void *contents, size_t size, size_t nmemb, void *userp)
{
  ...
  if(handleChange.retx_chunk_size>=MAX_CHUNK_SIZE) {
    ...
    //printf("\n C2:ChunkSize:%d",handleChange.retx_chunk_size);//handleChange.chunk_size);
    handleChange.retx_chunk_size=0;
  }
...
WriteMemoryCallback(void *contents, size_t size, size_t nmemb, void *userp)
{
  ...
  if(handleChange.chunk_size>=MAX_CHUNK_SIZE) {
    ...
    auto write_ret = future.get();
    //    printf("\n C1:ChunkSize:%d",handleChange.chunk_size);
...
int main(){
  ...
  do {
	...
	//cl_orig=10;
	if(cl_orig!=-1) {
	  content_len_orig= cl_orig;
	}
       }
      //which handle finished
      int msgq = 0;
	...
	//*multi_msg = NULL;
      ...
      if((handleChange.change==1)&&(current_handle!=NO_HANDLE)) {
	...
	if(current_handle == ORIG_EASY) {
	  ...
	  if(content_len_orig==0){
	    //handleChange.prev_run=handleChange.still_running;
	    break;
	  }
	}
	if(current_handle == RETX_EASY) {
	 
	  printf("\n--------------Retx_Size: %.0f---------------\n", content_len_retx);

	  if(content_len_retx==0){
	    //	    handleChange.prev_run=handleChange.still_running;
	    break;
	  }
	}
	...
	if(current_handle == ORIG_EASY) {
	  ...
	  auto write_ret = future.get();
	  //
	  string orig_chunk_size = std::to_string(content_len_orig);//cl_orig);
	...
	if(current_handle == RETX_EASY) {
	  ...
	  //
	  string retx_chunk_size = std::to_string(content_len_retx);//cl_retx);
	  read_exec = 2; //end of segment
