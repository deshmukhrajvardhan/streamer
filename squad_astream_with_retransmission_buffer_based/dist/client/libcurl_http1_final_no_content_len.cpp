...
int main(){
  ...
  if(data->features & CURL_VERSION_HTTP2){
    fprintf(stdout, "This libcurl DOES have HTTP2 support! But HTTP1 online\n");
  }
  ...
  do {
    ...
    if (read_ret_retx==1) {
      ...
      snprintf(retx_url, 1024, "%s", handleChange.url.c_str());
      ...
      curl_easy_setopt(easy[RETX_EASY], CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_1_1);
    ...
    //    printf("\nStill_running:%d, curr_retx:%d, curr_orig:%d\n",handleChange.still_running,current_retx_handle,current_orig_handle);
	  ...
	  //orig_done = 1;
            //    printf("\nMain1:Write Still_running:%d,Segment num:%d,Size:%zu",handleChange.still_running,++handleChange.seg_num,handleChange.chunk_size);
                ...
                //std::cout<<"\n(only multiperform) Download Rate:"<<(orig_content_len*8*1000)/(GetTimeMs64()-orig_multi_perf_to_done)<<"\n";
                //file_download_rate
		std::fstream("CppAppDwRate.txt",std::fstream::out|std::fstream::app)<< orig_content_len/1000000<< ',' << (orig_content_len*8*1000)/((GetTimeMs64()-orig_multi_perf_to_done)*(1000000))<< "\n";
	  ...
	  //retx_done = 1;
            //    printf("\nMain2:Write Still_running:%d,Segment num:%d,Size:%zu",handleChange.still_running,++handleChange.seg_num,handleChange.retx_chunk_size);
                ...
                //std::cout<<"\n{RETX}(only multiperform) Download Rate:"<<(retx_content_len*8*1000)/(GetTimeMs64()-retx_multi_perf_to_done)<<"\n";
                //file_download_rate<<
		std::fstream("Http1CppAppDwRate.txt",std::fstream::out|std::fstream::app)<<retx_content_len/1000000<< ',' << (retx_content_len*8*1000)/((GetTimeMs64()-retx_multi_perf_to_done)*(1000000))<< "\n";
