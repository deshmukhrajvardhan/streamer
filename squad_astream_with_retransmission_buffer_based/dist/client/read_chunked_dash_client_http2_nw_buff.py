...
#        connection.network_buffer_size= DOWNLOAD_CHUNK
    ...
    for mpd_data in mpd_conn.read_chunked():
        if mpd_data is b'':
            print("chunked:",len(t))
            exit()
            break
...
def download_segment(segment_url, dash_folder):
    ...
    try:
        ...
        with open(segment_filename,'wb') as segment_file_handle:
            for segment_data in seg_conn.read_chunked():
                    if segment_data is b'':
                        print("I'm OUT !!!!!!!!!")
                        with open('/mnt/QUIClientServer0/chunk_rate_read_chunked_squad_HTTP2.txt', 'a') as chk:
...
#                    with open('/mnt/QUIClientServer0/data_read_chunked_hyper_HTTP2','ab') as movie:
#                        movie.write(segment_data)
                    ...
                    print ("##################")
                    #        break
                        #segment_data=seg_conn.read(DOWNLOAD_CHUNK) #to continue reading chunks of info till the end of response, to be called at the end of loop
...
def start_playback_smart(dp_object, domain, playback_type=None, download=False, video_segment_duration=None, retrans=False):
    ...
    while segment_number < len(dp_list):
        ...
        with open('/mnt/QUIClientServer0/hyper_http2_read_chunked_seg_time_rate.txt', 'a') as rate_f:
            rate_f.write(str(segment_size)+'\t'+str(segment_download_time)+'\t'+str(segment_download_rate)+'\n')
