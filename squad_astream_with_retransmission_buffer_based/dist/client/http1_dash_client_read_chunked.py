...
#            print("!!!!saw b:",len(mpd_data),"chunks:",chunk_dl_rates)
#            exit()
#            break
...
def id_generator(id_size=6):
    """ Module to create a random string with uppercase 
        and digits.
    """
    return 'TEMP_' + ''.join(random.choice(ascii_letters+digits) for _ in range(id_size)) + str(timeit.default_timer())
...
#                    segment_data = seg_conn.read(DOWNLOAD_CHUNK)
        ...
        return #download_segment(segment_url, dash_folder)
    except:
        print("Unexpected error:", sys.exc_info())
        raise
#    except urllib.request.HTTPError as error:
#        config_dash.LOG.error("Unable to download file HTTP Error: %s" % error.code)
#        return None
    ...
    while segment_number < len(dp_list):
        ...
        if segment_number == dp_object.video[bitrate].start:
            ...
            if playback_type.upper() == "BASIC":
                current_bitrate, average_dwn_time = basic_dash2.basic_dash2(segment_number, bitrates, average_dwn_time, recent_download_sizes, previous_segment_times, current_bitrate)
                ...
                if not os.path.exists(download_log_file):
                            ...
                            stats = (timeit.default_timer()-start_dload_time, str(dash_player.buffer.__len__()), current_bitrate, segment_download_rate, segment_number,segment_size,segment_download_time)
                else:
                            header_row=None
                            stats = (timeit.default_timer()-start_dload_time, str(dash_player.buffer.__len__()), current_bitrate, segment_download_rate,segment_number,segment_size,segment_download_time)
                str_stats = [str(i) for i in stats]
        ...
        try:
            ...
            config_dash.LOG.error("Unable to save segment: {0}".format(e.filename))
            return None
        ...
        with open('/dev/SQUAD/http1_btw_Done_next_GET','a') as reqgap:
