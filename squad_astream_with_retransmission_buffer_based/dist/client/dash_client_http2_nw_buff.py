...
def download_segment(segment_url, dash_folder):
    ...
    try:
        ...
        with open(seg_dw_object.segment_filename,'wb') as segment_file_handle:
            ...
            while segment_data:
#                    with open('/dev/SQUAD/data_read_mod_chunk_hyper_HTTP2','ab') as movie:
#                        movie.write(segment_data)
             ...
             #       with open('/dev/SQUAD/hyper_http2_read_mod_chunk_time_rate.txt','a') as rate_f:
             #           rate_f.write(str(segment_size)+'\t'+str(total_data_dl_time)+'\t'+str(current_chunk_dl_rate)+'\n')
                    ...
                    if (len(segment_data) < DOWNLOAD_CHUNK):
                        with open('/dev/SQUAD/chunk_rate_read_mod_chunk_squad_HTTP2.txt','a') as chk:
                            chk.write("%s" %segment_url)
                            for item in seg_dw_object.segment_chunk_rates:
                                chk.write(",%s" %item)
                            chk.write("\n")
                        
                        #seg_dw_object.segment_chunk_rates.append(chunk_dl_rates)
              #          with open('/dev/SQUAD/hyper_http2_read_mod_chunk_time_rate.txt','a') as rate_f:
              #              rate_f.write("\n End of segment:{}\n".format(segment_url))
                        break
...
def retx_download_segment(retx_segment_url, dash_folder, retrans_next_segment_size, video_segment_duration, current_play_segment_number, current_retx_segment_number):
    ...
    #connection = HTTPConnectionPool(parse_url.netloc)
    #chunk_dl_rates = []
    #with open("/dev/SQUAD/retx_API_in_proof.txt",'a') as rtx_api_proof:
    ...
    try:
        ...
        seg_dw_remaining_time = 0
        time_remaining_till_playback = 0
        #seg_conn = connection.request('GET', segment_url) 
        #with closing(connection.get(segment_url, stream=True)) as seg_conn:
...
#                        movie.write(segment_data)
                    ...
                    current_chunk_dl_rate = retx_seg_dw_object.segment_size * 8 / total_data_dl_time
                     ...
                     #   if (seg_dw_remaining_time > time_remaining_till_playback):
                        #    with open("/dev/SQUAD/in_retx_retx_abandonment",'a') as retx_abandon:
                    ...
                    current_segment_chunk_rates.append(current_chunk_dl_rate)

                    
                    if (len(segment_data) < DOWNLOAD_CHUNK):
                        with open('/dev/SQUAD/retx_chunk_rate_read_mod_chunk_squad_HTTP2.txt','a') as chk:
                        ...
                        #with open("/dev/SQUAD/retx_not_abandonment",'a') as retx_not_abandon:
        ...
        config_dash.LOG.error("Unable to download DASH Segment {} HTTP Error:{} ".format(retx_segment_url, str(error.code)))
        return None
     #except requests.exceptions.ResponseNotChunked:
     #   return None
    ...
    seg_conn.close()
    segment_file_handle.close()
    #with open('/dev/SQUAD/retx_chunk_rate_read_mod_chunk_squad_HTTP2.txt','a') as retx_done:
                ...
                if segment_number > 10:
                    if current_bitrate <= bitrate_history[-1] or dash_player.buffer.__len__() < buffer_lower:
                        ...
                        if dash_player.buffer.__len__() >= buffer_upper: #MZ
                            ...
                            elif len(bitrates)>1 and current_bitrate < bitrates[int(bitrates.index(bitrate_history[-1]) - 2)]:
                            ...
                            elif len(bitrates)>1 and (current_bitrate >= bitrates[int(bitrates.index(bitrate_history[-1]) - 2)]) and (current_bitrate < bitrate_history[-1]):
                ...
                if segment_number > 10 and cmdline_retrans and retx_thread == False:
                    ...
                    if dash_player.buffer.__len__() >= (RETRANS_THRESHOLD_UPPER * config_dash.NETFLIX_BUFFER_SIZE) or RETRANSMISSION_SWITCH == True:
                        ...
                        if retx_flag:#segment_number != original_segment_number:
                            ...
                            retrans_next_segment_size = get_segment_sizes(dp_object,segment_number-2)[current_bitrate]
                        with open('empirical-debug.txt', 'a') as emp:
