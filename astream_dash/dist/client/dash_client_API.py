...
def download_retx_segment(segment_url, dash_folder):
    """ Module to download the retx_segment. No measuerments needed 
...
def start_playback_smart(dp_object, domain, playback_type=None, download=False, video_segment_duration=None, retrans=False):
    ...
    while segment_number < len(dp_list):
        if retransmission_delay_switch == True:
            ...
            retransmission_delay_switch = False
        segment = segment_number # as we download different seg, now need the original seg
        #print len(dp_list)
        #print "dp_list"
        #print segment
                ...
                if segment_number > 10 and retrans:
                    ...
                    if dash_player.buffer.__len__() >= (RETRANS_THRESHOLD_UPPER * config_dash.NETFLIX_BUFFER_SIZE) or RETRANSMISSION_SWITCH == True:
                        ...
                        if dash_player.buffer.__len__() < (RETRANS_THRESHOLD_LOWER * config_dash.NETFLIX_BUFFER_SIZE):
                            RETRANSMISSION_SWITCH = False # does it worl for the next segment?
        ...
        try:
            ...
            segment_size, segment_filename, segment_w_chunks = download_retx_segment(segment_url, file_identifier) if True else download_segment(segment_url, file_identifier)
        ...
        segment_number += 1 # next iter next seg
        if retransmission_delay_switch == True:
            segment_number = original_segment_number # as we download different seg, now need the original seg
        #if segment_number > 10:
