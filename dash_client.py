...
#    with open("/mnt/QUIClientServer0/REQUESTSgetraw.read",'a') as rread:
 #       rread.write(mpd_conn.raw.read()+"\n")
        ...
        with closing(connection.get(segment_url, stream=True)) as seg_conn:
            with open(segment_filename,'wb') as segment_file_handle:
                for segment_data in seg_conn.iter_content(DOWNLOAD_CHUNK):
                    #with open("/mnt/QUIClientServer0/data-REQUESTS-http1",'a') as rread:
                    ...
                    with open('/mnt/QUIClientServer0/requests_http1_read_mod_chunk_time_rate.txt','a') as rate_f:
			    ...
			    chk.write("\n")
                        ...
                        with open('/mnt/QUIClientServer0/requests_http1_read_mod_chunk_time_rate.t\
xt', 'a') as rate_f:
                            rate_f.write("\n End of segment:{}\n".format(segment_url))
                        #print "##################"
                        break
    ...
    while segment_number < len(dp_list):
		...
		if not os.path.exists(download_log_file):
	        ...
	        if not os.path.exists(download_log_file):
                ...
                if segment_size and segment_download_time:
                    segment_download_rate = segment_size / segment_download_time
                    #with open('/mnt/QUIClientServer0/http1_read_seg_size_rate.txt', 'a') as rate_f:
        ...
        segment_download_rate=segment_size/segment_download_time
        with open('/mnt/QUIClientServer0/requests_http1_iter_seg_size_time_rate.txt', 'a') as rate_f:
